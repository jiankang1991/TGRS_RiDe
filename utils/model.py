...
class BNInception(nn.Module):
    ...
    def forward(self, x, with_feature = False):
        ...
        x = F.adaptive_max_pool2d(feature, output_size=1)
        x = x.view(x.size(0), -1)
        if self.dim == 0:
            return x
        x, origin = self.classifier(x)
        if with_feature:
            return x, feature, origin
        else:
            return x
...
if __name__ == "__main__":
    ...
    model = BNInception(dim=128)
    # model = ResNet34(dim=128)

    inputs = torch.randn((1,3,256,256))

    # with torch.no_grad():
        # outputs, f, orig = model(inputs, True)
        # outputs = model(inputs)

    # print(outputs.shape, f.shape, orig.shape)
    # print(outputs.shape)

    outputs, f, orig = model(inputs, True)

    print(outputs.is_leaf)
    print(f.is_leaf)
    print(orig.is_leaf)
